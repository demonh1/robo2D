...
SupportSpotCalculator::SupportSpotCalculator(int           numX,
                                             ...
                                             SoccerTeam*   team):m_pBestSupportingSpot(NULL),
                                                                  m_pTeam(team)
{
  const Region* PlayingField = team->getPitch()->PlayingArea();
  ...
  for (int x=0; x<(numX/2)-1; ++x)
  {
    ...
    {      
      if (m_pTeam->getColor() == SoccerTeam::blue)
      {
...
Vector2D SupportSpotCalculator::DetermineBestSupportingPosition()
{
  ...
  for (curSpot = m_Spots.begin(); curSpot != m_Spots.end(); ++curSpot)
  {
    ...
    //to this position?
	if(m_pTeam->isPassSafeFromAllOpponents(m_pTeam->getControllingPlayer()->getPosition(),
                                           curSpot->m_vPos,
                                           NULL,
                                           Prm.MaxPassingForce))
    {
    ...
    if (m_pTeam->getSupportingPlayer())
    {
	  ...
	  double dist = Vec2DDistance(m_pTeam->getControllingPlayer()->getPosition(),
                                 curSpot->m_vPos);
