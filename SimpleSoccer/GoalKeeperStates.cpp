...
bool GlobalKeeperState::OnMessage(GoalKeeper* keeper, const Message& telegram)
{
...
void PutBallBackInPlay::Enter(GoalKeeper* keeper)
{
  ...
  keeper->Team()->getOpponents()->ReturnAllFieldPlayersToHome();
...
void PutBallBackInPlay::Execute(GoalKeeper* keeper)
{
  ...
  {     
    //make the pass   
	  keeper->Ball()->Kick(Vec2DNormalize(BallTarget - keeper->Ball()->getPosition()),
                         Prm.MaxPassingForce);
