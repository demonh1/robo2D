...
//  std::vector<PlayerBase*>::iterator it = m_Players.begin();
  ...
  for (std::vector<PlayerBase*>::iterator it = m_Players.begin(); it != m_Players.end(); ++it)
  {
    //calculate the dist. Use the squared value to avoid sqrt
	  double dist = Vec2DDistanceSq((*it)->getPosition(), getPitch()->Ball()->getPosition());
...
PlayerBase* SoccerTeam::DetermineBestSupportingAttacker()
{
  ...
  for (std::vector<PlayerBase*>::iterator it = m_Players.begin(); it != m_Players.end(); ++it)
  {
    ...
    {
      //calculate the dist. Use the squared value to avoid sqrt
		double dist = Vec2DDistanceSq((*it)->getPosition(), m_pSupportSpotCalc->GetBestSupportingSpot());
...
{  
  ...
  {   
    ...
    if ( (*curPlyr != passer) &&            
		(Vec2DDistanceSq(passer->getPosition(), (*curPlyr)->getPosition()) > 
...
{  
  ...
  double time = getPitch()->Ball()->TimeToCoverDistance(getPitch()->Ball()->getPosition(),
  ...
  GetTangentPoints(receiver->getPosition(),
                   InterceptRange,
                   getPitch()->Ball()->getPosition(),
                   ip1,
                   ip2);
 
  const int NumPassesToTry = 3;
  Vector2D Passes[NumPassesToTry] = {ip1, receiver->getPosition(), ip2};
  ...
  {    
    ...
    if (( dist < ClosestSoFar) &&
        ...
        isPassSafeFromAllOpponents(getPitch()->Ball()->getPosition(),
                                   Passes[pass],
                                   receiver,
                                   power))
                                        ...
                                        double       PassingForce)const
{
  ...
  Vector2D LocalPosOpp = PointToLocalSpace(opp->getPosition(),
                                         ToTargetNormalized,
  ...
  if (Vec2DDistanceSq(from, target) < Vec2DDistanceSq(opp->getPosition(), from))
  {
    if (receiver)
    {
		if ( Vec2DDistanceSq(target, opp->getPosition())  > 
			Vec2DDistanceSq(target, receiver->getPosition()) )
      {
        return true;
      }
  ...
  double TimeForBall = 
  getPitch()->Ball()->TimeToCoverDistance(Vector2D(0,0),
                                       Vector2D(LocalPosOpp.x, 0),
                                       PassingForce);

  //now calculate how far the opponent can run in this time
  double reach = opp->getMaxSpeed() * TimeForBall +
                getPitch()->Ball()->getBoundingRadius()+
...
bool SoccerTeam::isPassSafeFromAllOpponents(Vector2D                from,
                                            ...
                                            double     PassingForce)const
{
  std::vector<PlayerBase*>::const_iterator opp = getOpponents()->Members().begin();

  for (opp; opp != getOpponents()->Members().end(); ++opp)
  {
                          ...
                          Vector2D& ShotTarget)const
{
  ...
  while (NumAttempts--)
  {
    ...
    int MinYVal = OpponentsGoal()->LeftPost().y + getPitch()->Ball()->getBoundingRadius();
    int MaxYVal = OpponentsGoal()->RightPost().y - getPitch()->Ball()->getBoundingRadius();

    ShotTarget.y = (double)RandInt(MinYVal, MaxYVal);

    //make sure striking the ball with the given power is enough to drive
    //the ball over the goal line.
    double time = getPitch()->Ball()->TimeToCoverDistance(BallPos,
                                                      ShotTarget,
                                                      power);
  ...
  for (it; it != m_Players.end(); ++it)
  {
    if ((*it)->Role() != PlayerBase::goal_keeper)
    {
      /*Dispatcher->DispatchMsg(SEND_MSG_IMMEDIATELY,
                            ...
                            Msg_GoHome,
                            NULL);*/
		Dispatcher ->dispatchMessage( SEND_MSG_IMMEDIATELY, 1, (*it)->getAgentId(),
			msg::msgGoHome, NULL);
    }
  }
}
...
void SoccerTeam::Render()const
{
  ...
  if (Prm.bShowControllingTeam)
  {
    ...
    if ( (getColor() == blue) && inControl())
    {
      gdi->TextAtPos(20,3,"Blue in Control");
    }
    else if ( (getColor() == red) && inControl())
    {
      gdi->TextAtPos(20,3,"Red in Control");
    }
    if (m_pControllingPlayer != NULL)
    {
      gdi->TextAtPos(getPitch()->cxClient()-150, 3, "Controlling Player: " + ttos(m_pControllingPlayer->getAgentId()));
    }
  }
...
void SoccerTeam::RequestPass(FieldPlayer* requester)const
{
                                 ...
                                 Prm.MaxPassingForce))
  {
   ...
   /* Dispatcher->DispatchMsg(SEND_MSG_IMMEDIATELY,
                          requester->getAgentId(),
						  ControllingPlayer()->getAgentId(),
                          Msg_PassToMe,
                          requester); 
*/
	  Dispatcher->dispatchMessage( SEND_MSG_IMMEDIATELY, requester->getAgentId(),
		  getControllingPlayer()->getAgentId(), msg::msgPassToMe, requester);
  }
}
...
{
  std::vector<PlayerBase*>::const_iterator end = getOpponents()->Members().end();
  std::vector<PlayerBase*>::const_iterator it;

  for (it = getOpponents()->Members().begin(); it != end; ++it)
  {
	  if (Vec2DDistanceSq(pos, (*it)->getPosition()) < rad*rad)
    {
      return true;
    }
  }
