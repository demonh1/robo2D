...
{  
  ...
  Vector2D acceleration = (direction * force) / m_Mass;
...
void SoccerBall::Update()
{
  ...
  //first though
  if (m_Velocity.LengthSq() > Prm.Friction * Prm.Friction)
  {
    m_Velocity += Vec2DNormalize(m_Velocity) * Prm.Friction;
...
Vector2D SoccerBall::FuturePosition(double time)const
{
  ...
  Vector2D ScalarToVector = half_a_t_squared * Vec2DNormalize(m_Velocity);

  //the predicted position is the balls position plus these two terms
  return getPosition() + ut + ScalarToVector;
}
...
void SoccerBall::Render()
{
  ...
  gdi->Circle(m_Position, m_BoundingRadius);
...
{  
  ...
  Vector2D VelNormal = Vec2DNormalize(m_Velocity);
  ...
  for (unsigned int w=0; w<walls.size(); ++w)
  {
    ...
    Vector2D ThisCollisionPoint = getPosition() - (walls[w].Normal() * getBoundingRadius());
    ...
    if ((distSq <= m_Velocity.LengthSq()) && (distSq < DistToIntersection) && OnLineSegment)            
    {        
  ...
  {
    m_Velocity.Reflect(walls[idxClosest].Normal());   
  }
}
