...
{ 
  ...
  Vec2DRotateAroundOrigin(m_Heading, TurningForce);

  //make sure the velocity vector points in the same direction as
  //the heading vector
  m_Velocity = m_Heading * m_Velocity.Length();
  ...
  Vector2D accel = m_Heading * m_pSteering->ForwardComponent() / m_Mass;
...
void FieldPlayer::Render()                                         
{
  ...
  if (Team()->getColor() == SoccerTeam::blue){gdi->BluePen();}
  else{gdi->RedPen();}
  ...
  //and 'is 'ead
  gdi->BrownBrush();
  if (Prm.bHighlightIfThreatened && (Team()->getControllingPlayer() == this) && isThreatened()) gdi->YellowBrush();
  ...
  {
    gdi->RedBrush();
    gdi->Circle(Steering()->getTarget(), 3);
    gdi->TextAtPos(Steering()->getTarget(), ttos(getAgentId()));
  }   
}
