...
bool GlobalPlayerState::OnMessage(FieldPlayer* player, const Message& telegram)
{
  switch(telegram.getiMessage())
  {
  case  msg::msgReceiveBall:
    {
      //set the target
      player->Steering()->setTarget(*(static_cast<Vector2D*>(telegram.inform)));
  ...
  case msg::msgPassToMe:
    {  
      
      //get the position of the player requesting the pass 
      FieldPlayer* receiver = static_cast<FieldPlayer*>(telegram.inform);

      #ifdef PLAYER_STATE_INFO_ON
      debug_con << "Player " << player->getAgentId() << " received request from " <<
                    receiver->getAgentId() << " to make pass" << "";
      #endif

      //if the ball is not within kicking range or their is already a 
      //receiving player, this player cannot pass the ball to the player
      //making the request.
      if (player->Team()->getReceiver() != NULL ||
         !player->BallWithinKickingRange() )
      {
        #ifdef PLAYER_STATE_INFO_ON
        debug_con << "Player " << player->getAgentId() << " cannot make requested pass <cannot kick ball>" << "";
        #endif
      ...
      //make the pass   
	  player->Ball()->Kick(receiver->getPosition() - player->Ball()->getPosition(),
                           Prm.MaxPassingForce);

          
     #ifdef PLAYER_STATE_INFO_ON
     debug_con << "Player " << player->getAgentId() << " Passed ball to requesting player" << "";
     #endif
...
void ChaseBall::Enter(FieldPlayer* player)
{
  ...
  #ifdef PLAYER_STATE_INFO_ON
  debug_con << "Player " << player->getAgentId() << " enters chase state" << "";
  #endif
}
...
void ChaseBall::Execute(FieldPlayer* player)                                     
{
  ...
  {
    player->Steering()->setTarget(player->Ball()->getPosition());
...
void SupportAttacker::Enter(FieldPlayer* player)
{
  ...
  #ifdef PLAYER_STATE_INFO_ON
  debug_con << "Player " << player->getAgentId() << " enters support state" << "";
  #endif
}
...
void SupportAttacker::Execute(FieldPlayer* player)                                     
{
  ...
  if( player->Team()->CanShoot(player->getPosition(),
...
void ReturnToHomeRegion::Enter(FieldPlayer* player)
{
  ...
  if (!player->HomeRegion()->Inside(player->Steering()->getTarget(), Region::halfsize))
  {
    player->Steering()->setTarget(player->HomeRegion()->Center());
  }

  #ifdef PLAYER_STATE_INFO_ON
  debug_con << "Player " << player->getAgentId() << " enters ReturnToHome state" << "";
  #endif
}
...
void ReturnToHomeRegion::Execute(FieldPlayer* player)
{
  ...
  if (player->Pitch()->GameOn() && player->HomeRegion()->Inside(player->getPosition(),
                                                             Region::halfsize))
  {
...
{
  #ifdef PLAYER_STATE_INFO_ON
  debug_con << "Player " << player->getAgentId() << " enters wait state" << "";
  #endif
...
void KickBall::Enter(FieldPlayer* player)
{
  ...
  #ifdef PLAYER_STATE_INFO_ON
  debug_con << "Player " << player->getAgentId() << " enters kick state" << "";
  #endif
}
...
{ 
  ...
  //and the player's heading
	Vector2D ToBall = player->Ball()->getPosition() - player->getPosition();
  ...
  //to make the shot
  if (player->Team()->CanShoot(player->Ball()->getPosition(),
                               power,
                               BallTarget)                   || 
     (RandFloat() < Prm.ChancePlayerAttemptsPotShot))
  {
   #ifdef PLAYER_STATE_INFO_ON
   debug_con << "Player " << player->getAgentId() << " attempts a shot at " << BallTarget << "";
   #endif

   //add some noise to the kick. We don't want players who are 
   //too accurate! The amount of noise can be adjusted by altering
   //Prm.PlayerKickingAccuracy
   BallTarget = AddNoiseToKick(player->Ball()->getPosition(), BallTarget);

   //this is the direction the ball will be kicked in
   Vector2D KickDirection = BallTarget - player->Ball()->getPosition();
  ...
  {     
    ...
    #ifdef PLAYER_STATE_INFO_ON
    debug_con << "Player " << player->getAgentId() << " passes the ball with force " << power << "  to player " 
              << receiver->getAgentId() << "  Target is " << BallTarget << "";
    #endif
...
#ifdef PLAYER_STATE_INFO_ON
  debug_con << "Player " << player->getAgentId() << " enters dribble state" << "";
  #endif
}

void Dribble::Execute(FieldPlayer* player)
{
  double dot = player->Team()->HomeGoal()->Facing().Dot(player->getHeading());
  ...
  if (dot < 0)
  {
    ...
    double angle = QuarterPi * -1 *
                 player->Team()->HomeGoal()->Facing().Sign(player->getHeading());
...
void ReceiveBall::Enter(FieldPlayer* player)
{
		...
		!player->Team()->isOpponentWithinRadius(player->getPosition(), PassThreatRadius))
  {
    ...
    #ifdef PLAYER_STATE_INFO_ON
    debug_con << "Player " << player->getAgentId() << " enters receive state (Using Arrive)" << "";
    #endif
  }
  else
  {
    ...
    #ifdef PLAYER_STATE_INFO_ON
    debug_con << "Player " << player->getAgentId() << " enters receive state (Using Pursuit)" << "";
    #endif
  }
}
