...
bool SteeringBehaviors::AccumulateForce(Vector2D &sf, Vector2D ForceToAdd)
{
  ...
  double magnitudeRemaining = m_pPlayer->getMaxForce() - MagnitudeSoFar;
...
Vector2D SteeringBehaviors::Seek(Vector2D target)
{
	...
	Vector2D DesiredVelocity = Vec2DNormalize(target - m_pPlayer->getPosition())
...
{
	Vector2D ToTarget = target - m_pPlayer->getPosition();
  ...
  if (dist > 0)
  {
    ...
    speed = min(speed, m_pPlayer->getMaxSpeed());
...
{
	Vector2D ToBall = ball->getPosition() - m_pPlayer->getPosition();
...
void SteeringBehaviors::FindNeighbours()
{
  ...
  for (curPlyr = AllPlayers.begin(); curPlyr!=AllPlayers.end(); ++curPlyr)
  {
	...
	Vector2D to = (*curPlyr)->getPosition() - m_pPlayer->getPosition();
...
{  
  ...
  for (curPlyr = AllPlayers.begin(); curPlyr!=AllPlayers.end(); ++curPlyr)
  {
    ...
    {
		Vector2D ToAgent = m_pPlayer->getPosition() - (*curPlyr)->getPosition();
...
{
  return Arrive(target + Vec2DNormalize(ball->getPosition() - target) * 
...
{ 
  ...
  gdi->Line(m_pPlayer->getPosition(), m_pPlayer->getPosition() + m_vSteeringForce * 20);
